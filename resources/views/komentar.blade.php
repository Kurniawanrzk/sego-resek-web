...
<div class="container-komen mt-3 p-5"  id="komen">
    ...
    <a href="{{Route("home"). "#kirimkomentar"}}">    <button class="btn btn-light ms-1">Tambah Komentar</button>
    </a>
    <div>
        @foreach ($with["komentar"] as $item )
            <div class="bg-light p-3 rounded shadow mt-2" style="min-width:100%;height:fit-content">
                <p>{{$item->komen}}</p>
                <p style="color:rgb(103, 103, 194)">{{ \Carbon\Carbon::parse($item->waktu_komentar)->diffForHumans() }}</p>
            </div>
           ...
           ->get();
            @endphp
               ...
               <div style="background-color: rgb(248, 238, 211)" class="ms-5 p-3 rounded shadow mt-2" style="height:fit-content">
                <p>{{$balasan->balasan}}<br><i style="color:grey">By Admin.</i><br><span style="color:rgb(103, 103, 194)">{{ \Carbon\Carbon::parse($balasan->waktu_balas)->diffForHumans() }}</span></p>
            </div>
               @endforeach
            @endif
        @endforeach
